...
const userSettings = {
	...
	layout: {
		...
		hideSystemInformation: false,
		swapAsciiSystemInformation: false,
		asciiAlignment: "left",
	...
	times: [
		{ enabled: true, label: "HOME", timezone: 0 },
...
window.wallpaperPropertyListener = {
	applyUserProperties: function (properties) {
		...
		if (properties.swapasciisysteminformation) {
			userSettings.layout.swapAsciiSystemInformation = Boolean(properties.swapasciisysteminformation.value);
			shouldUpdateLayout = true;
		}
		...
		if (properties.hidesysteminformation) {
			userSettings.layout.hideSystemInformation = Boolean(properties.hidesysteminformation.value);
			shouldUpdateLayout = true;
		}

		if (properties.asciiartalignment) {
			userSettings.layout.asciiAlignment = properties.asciiartalignment.value;
			shouldUpdateLayout = true;
		}
		...
		for (let i = 1; i <= 3; i++) {
			if (properties[`info-default-divider-${i}`]) {
				const property = properties[`info-default-divider-${i}`];

				document.querySelector(`.separator-${i}`).style = property.value === true ? "" : "display: none";
			}
		}
...
function updateLayout() {
	...
	const infoPanelEl = document.querySelector(".overview-terminal .information");
	...
	if (userSettings.layout.hideSystemInformation === true) {
		infoPanelEl.classList.add("no-text");

		const alignment = userSettings.layout.asciiAlignment;
		infoPanelEl.style = `justify-content: ${alignment}`;
	} else {
		infoPanelEl.classList.remove("no-text");
	}

	if (userSettings.layout.swapAsciiSystemInformation) {
		infoPanelEl.classList.add("reverse");
	} else {
		infoPanelEl.classList.remove("reverse");
	}
...
function updateSystem() {
	...
	for (let i = 1; i <= customSystemInfoValues; i++) {
		if (
			...
			if (userSettings.customSystemInfo[`value-${i}`] === "{resolution}") document.querySelector(`.custom-value-${i}`).textContent = `${document.body.scrollWidth}x${document.body.scrollHeight}`;
			else document.querySelector(`.custom-value-${i}`).textContent = userSettings.customSystemInfo[`value-${i}`];
